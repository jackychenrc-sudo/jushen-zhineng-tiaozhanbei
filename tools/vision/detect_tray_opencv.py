#!/usr/bin/env python3
"""
Scene3 tray OpenCV baseline detector.

Input:
  A saved RGB image, usually from /cam_h/color/image_raw/compressed.

Output:
  Console candidate bboxes and a visualization image.
"""
...
def draw_candidates(
    ...
    cv2.putText(vis, "ROI", (roi_box[0], max(20, roi_box[1] - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2, cv2.LINE_AA)
    ...
    for i, cand in enumerate(candidates[:max_draw]):
        ...
        cv2.putText(vis, f"{i}: ({u},{v})", (x1, max(18, y1 - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)
    return vis
...
def main() -> None:
    ...
    parser.add_argument("--output", default=None, help="output visualization image")
    parser.add_argument("--roi", default="0.25,0.15,0.75,0.78", help="normalized ROI x1,y1,x2,y2")
    ...
    for i, cand in enumerate(candidates[:20]):
        print(f"candidate_{i}: bbox={list(cand.bbox)} center={list(cand.center)} area={cand.area:.1f} score={cand.score:.1f}")

    output_path = Path(args.output) if args.output else image_path.with_name(image_path.stem + "_tray_detect.jpg")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    cv2.imwrite(str(output_path), draw_candidates(image, candidates, roi=roi))
