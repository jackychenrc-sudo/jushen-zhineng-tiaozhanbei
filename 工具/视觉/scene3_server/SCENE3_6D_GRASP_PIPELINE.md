...
> **当前控制结论（2026-07-17）**：本分支仅使用已通过小段笛卡尔运动验证的
> `/kuavo_arm_traj`。不要把执行端替换为 `/kuavo_arm_target_poses`；一次尚未合入
> 的本地实验中，规划的 5 mm TCP 小段产生了约 51 mm 实际运动，并带动了本应
> 保持的左臂。详细证据、已排除原因和恢复路线见
> [SCENE3_ARM_CONTROL_INCIDENT.md](SCENE3_ARM_CONTROL_INCIDENT.md)。
