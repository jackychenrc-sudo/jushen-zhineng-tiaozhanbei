...
class Scene3Task(object):
    ...
    def solve_right_hand_ik(self, right_pos_xyz, right_quat_xyzw):
        ...
        # Match the request format already verified by the existing Scene3
        # reachability scan: an explicit base_link frame and current q0.
        req.hand_poses.header.frame_id = "base_link"
    ...
    def grasp_tray_from_latest_target(self):
        """Run the fail-closed arm gate; never close the claw in v1."""
        from scene3_arm_guard import (
            PREGRASP_CONFIRMATION,
            Scene3ArmGuard,
            write_report,
        )

        stage = os.environ.get("SCENE3_ARM_STAGE", "analysis").strip().lower()
        if stage not in ("analysis", "pregrasp"):
            self.rospy.logerr(
                "scene3: unsupported SCENE3_ARM_STAGE=%s; use analysis or pregrasp",
                stage,
            )
            return False
        confirmation = os.environ.get("SCENE3_ARM_CONFIRMATION", "")
        execute_pregrasp = stage == "pregrasp"
        if execute_pregrasp and confirmation != PREGRASP_CONFIRMATION:
            self.rospy.logwarn(
                "scene3: pregrasp requested without exact confirmation; arm remains blocked"
            )

        guard = Scene3ArmGuard(self)
        report = guard.run(
            execute_pregrasp=execute_pregrasp,
            confirmation=confirmation,
        )
        output_path = os.environ.get(
            "SCENE3_ARM_REPORT", "/tmp/scene3_arm_guard.json"
        )
        try:
            write_report(report, output_path)
        except Exception as exc:
            self.rospy.logwarn("scene3: failed to save arm guard report: %s", exc)
        self.rospy.loginfo(
            "scene3: arm guard status=%s report=%s",
            report.get("status"),
            output_path,
        )
        if report.get("status") == "single_pregrasp_completed":
            self.rospy.loginfo(
                "scene3: single pregrasp completed; stop for visual inspection"
            )
        else:
            self.rospy.logwarn(
                "scene3: arm stage stopped before touch/claw: %s",
                report.get("next_step", report.get("error_reason", "")),
            )

        # A completed pregrasp is not a completed grasp.  Keep the transport
        # branch disabled until touch, claw-state verification and lift pass.
        return False
