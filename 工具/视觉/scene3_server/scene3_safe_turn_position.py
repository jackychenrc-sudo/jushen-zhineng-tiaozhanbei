...
def command_reference_error_deg(measured_radians, reference_degrees):
    return maximum_abs([
        math.degrees(float(measured_radians[index]))
        - float(reference_degrees[index])
        for index in range(ARM_COUNT)
    ])


def low_level_reference_error_deg(low_level_radians, reference_degrees):
    return maximum_abs([
        math.degrees(float(low_level_radians[index]))
        - float(reference_degrees[index])
        for index in range(ARM_COUNT)
    ])


def choose_handover_reference(
        saved_reference_degrees,
        measured_radians,
        maximum_saved_error_degrees):
    error = command_reference_error_deg(
        measured_radians,
        saved_reference_degrees,
    )
    if error > float(maximum_saved_error_degrees):
        return (
            [math.degrees(float(value)) for value in measured_radians],
            True,
            error,
        )
    return (
        [float(value) for value in saved_reference_degrees],
        False,
        error,
    )
...
def build_parser():
    ...
    parser.add_argument("--maximum-baseline-error-deg", type=float, default=3.0)
    parser.add_argument("--maximum-arm-drift-deg", type=float, default=0.15)
    parser.add_argument("--maximum-handover-source-error-deg", type=float, default=1.0)
    parser.add_argument("--maximum-handover-drift-deg", type=float, default=0.75)
    parser.add_argument(
        "--maximum-low-level-reference-error-deg",
        type=float,
        default=0.50,
    )
    parser.add_argument("--minimum-right-arm-kp", type=float, default=0.1)
    parser.add_argument("--handover-seconds", type=float, default=1.5)
...
def run_ros(args):
    ...
    def sample_low_level(count=5):
        ...
        positions = [
            statistics.median(row[index] for row in position_samples)
            for index in range(ARM_COUNT)
        ]
        modes = None
        if mode_samples:
            modes = [
                int(statistics.median(row[index] for row in mode_samples))
                for index in range(ARM_COUNT)
            ]
        gains = None
        if gain_samples:
            gains = [
                statistics.median(row[index] for row in gain_samples)
                for index in range(ARM_COUNT)
            ]
    ...
    def call_mode_service(service_name, control_mode):
        ...
        if not getattr(response, "result", False):
            raise RuntimeError(
                "{} rejected mode {}".format(service_name, control_mode)
            )

    def wait_internal_arm_ready():
        mode_ready = False
        mode_values = []
        ...
        return mode_ready and abs(reset_value) <= 0.1

    def prepare_command_reference(saved_reference):
        measured_before, mapping, measured_drift = sample_arm(count=7)
        candidate, rebased, source_error = choose_handover_reference(
            saved_reference,
            measured_before,
            args.maximum_handover_source_error_deg,
        )
        print("Preparing one-time 14-joint command handover")
        print("Joint mapping: {}".format(mapping))
        print("Saved-reference error: {:.3f}deg".format(source_error))
        print("Measured arm drift before handover: {:.3f}deg".format(
            measured_drift
        ))
        if measured_drift > args.maximum_arm_drift_deg:
            print("ARM_COMMAND_HANDOVER_BLOCKED: measured arm is moving")
            return None
        if rebased:
            print("HANDOVER_SOURCE_REBASED_TO_STABLE_MEASUREMENT")

        # Merely changing the ROS parameter is not a control handover.  First
        # publish the complete safe reference, then enable the WBC trajectory
        # path while continuing to hold the same target.
        rospy.set_param(REFERENCE_PARAM, list(candidate))
        hold_reference(candidate, args.handover_seconds)
        call_mode_service("/enable_wbc_arm_trajectory_control", 1)
        hold_reference(candidate, args.handover_seconds)

        measured_after, _, settled_drift = sample_arm(count=7)
        low_level, modes, gains = sample_low_level(count=7)
        handover_motion = maximum_abs([
            math.degrees(measured_after[index] - measured_before[index])
            for index in range(ARM_COUNT)
        ])
        measured_error = command_reference_error_deg(
            measured_after,
            candidate,
        )
        low_level_error = low_level_reference_error_deg(
            low_level,
            candidate,
        )
        right_modes_active = (
            modes is not None
            and all(int(value) == 2 for value in modes[RIGHT_ARM_START:])
        )
        right_gains_active = (
            gains is not None
            and all(
                float(value) > float(args.minimum_right_arm_kp)
                for value in gains[RIGHT_ARM_START:]
            )
        )
        checks = {
            "internal_mode_ready": wait_internal_arm_ready(),
            "handover_motion_bounded": (
                handover_motion <= args.maximum_handover_drift_deg
            ),
            "measured_reference_aligned": (
                measured_error <= args.maximum_handover_drift_deg
            ),
            "joint_cmd_aligned": (
                low_level_error
                <= args.maximum_low_level_reference_error_deg
            ),
            "right_modes_active": right_modes_active,
            "right_gains_active": right_gains_active,
            "arm_settled": settled_drift <= args.maximum_arm_drift_deg,
        }
        print("Handover physical motion: {:.3f}deg".format(handover_motion))
        print("Handover measured-reference error: {:.3f}deg".format(
            measured_error
        ))
        print("Handover /joint_cmd-reference error: {:.3f}deg".format(
            low_level_error
        ))
        print("Handover checks: {}".format(checks))
        if not all(checks.values()):
            hold_reference(candidate, args.handover_seconds)
            print("ARM_COMMAND_HANDOVER_BLOCKED: no Cartesian motion attempted")
            return None

        rospy.set_param(REFERENCE_PARAM, list(candidate))
        print("ARM_COMMAND_HANDOVER_OK")
    ...
    if args.handover_only and not execute:
        raise RuntimeError("--handover-only requires --execute")
    ...
    if execute:
        ...
        for service_name in (
            ...
            try:
                call_mode_service(service_name, 2)
                mode_ok = True
        ...
        reference = prepare_command_reference(reference)
        if reference is None:
            return 2
        if args.handover_only:
            print("ARM_COMMAND_HANDOVER_ONLY_OK")
            print("No Cartesian segment, base, or claw command was sent")
            return 0
    ...
    for segment_index in range(1, segment_limit + 1):
        ...
        try:
            ...
            after_low, after_modes, after_gains = sample_low_level()
            rospy.sleep(0.25)
            ...
            checks["joint_cmd_aligned"] = (
                low_level_error
                <= args.maximum_low_level_reference_error_deg
            )
            checks["right_modes_active"] = (
                after_modes is not None
                and all(
                    int(value) == 2
                    for value in after_modes[RIGHT_ARM_START:]
                )
            )
            checks["right_gains_active"] = (
                after_gains is not None
                and all(
                    float(value) > float(args.minimum_right_arm_kp)
                    for value in after_gains[RIGHT_ARM_START:]
                )
            )
            ...
            print("/joint_cmd-reference error: {:.3f}deg".format(
                low_level_error
            ))
