# GitHub 协作规则

这份文档只规定协作边界、分支和文件放置位置，不限制具体实现方法。熟悉 Git 的同学按自己的习惯操作；不熟悉的同学可以用 GitHub 网页或 GitHub Desktop。

## 分支命名

每个人不要直接改 `main`，使用自己的分支。

| 成员 | 分支名 |
| --- | --- |
| 闫帅辰 | `vision-yanshuaichen` |
| 王森 | `scene3-wangsen` |
| 王仔俊 | `scene1-wangzijun` |
| 史智涛 | `scene2-shizhitao` |
| Kevin | `scene2-kevin` |
| 窦欣悦 | `docs-douxinyue` |

PR 标题可以写中文，例如：

```text
Scene3：王森更新动作骨架
视觉：闫帅辰更新料盘定位结果
测试：窦欣悦更新测试记录
```

## 文件放置位置

| 文件类型 | 放到哪里 |
| --- | --- |
| Scene1 代码、说明 | `任务代码参考/场景一_包裹称重入箱/` |
| Scene2 代码、说明 | `任务代码参考/场景二_零件分拣/` |
| Scene3 代码、说明 | `任务代码参考/场景三_SMT料盘出库/` |
| 视觉工具脚本 | `工具/视觉/` |
| 视觉/坐标接口说明 | `任务代码参考/视觉与坐标/` |
| 每天的小成果 | `进度成果/日期/scene名/` |
| 测试记录、进度汇总 | `文档/` |
| 最终提交说明 | `最终提交包/` |

小成果包括小截图、JSON、小日志、说明文档。大视频、rosbag、模型权重不要传 GitHub。

## 每天同步节奏

```text
早上：确认自己分支是最新的。
白天：在自己负责范围内自由实现。
晚上：提交自己的分支，说明今天做了什么、证据在哪里、卡点是什么。
```

## 负责范围

```text
Scene1 组：任务代码参考/场景一_包裹称重入箱
Scene2 组：任务代码参考/场景二_零件分拣
Scene3 组：任务代码参考/场景三_SMT料盘出库
视觉组：任务代码参考/视觉与坐标、工具/视觉
测试同学：文档、最终提交包、进度成果
```

这些目录只是协作参考区。真正接入比赛时，最终代码仍然要整理进官方 `challenge_cup_task_template` 功能包。

## 不要提交这些

```text
build/
devel/
log/
*.bag
*.mp4
*.avi
*.mov
*.pt
*.pth
大型数据集
大模型权重
```

大文件放网盘或服务器路径，只在测试记录里写清楚位置。

## 每晚合并标准

能合并到 `main` 的内容应满足：

- 说得清楚改了什么。
- 有运行结果、截图、视频、日志或输出文件之一。
- 不影响其他人已有工作。
- 不包含大文件或临时文件。
- 如果没有验证，需要明确写“未验证原因”。

## 关键原则

不要多人长期同时改同一个大文件。最终接入官方入口时再统一整合，平时尽量按 scene 或模块拆开。